...
export function SignupPage({ onClose, onLoginClick }: SignupPageProps) {
  ...
  // ── Phase 4: 초대 코드 처리 ────────────────────────────────
  const handleInviteConnect = async (userEmail: string) => {
    ...
    try {
      ...
      // 연결 실패해도 가입은 완료 상태이므로 에러 무시
      console.error('초대 코드 처리 실패 (가입은 완료):', err);
    }
  };
  // ──────────────────────────────────────────────────────────
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // ── Phase 4: 초대 코드가 있으면 트레이너 연결
        await handleInviteConnect(formData.email);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <option value="gmu">George Mason University (조지메이슨대학교)</option>
                <option value="stony">Stony Brook University (스토니브룩대학교)</option>
                <option value="ghent">Ghent University (겐트대학교)</option>
                <option value="fit">Fashion Institute of Technology</option>
              </select>
            </div>
            ...
            {/* 추천인 */}
            <div className="space-y-2">
              <Label htmlFor="referrer">추천인 ID (선택)</Label>
              <Input
                id="referrer"
                name="referrer"
                type="text"
